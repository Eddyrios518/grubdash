...
function bodyHasMobileNumber(req, res, next) {
    ...
    //if there is a phone number associated with the order, move onto the next function
    if (mobileNumber) {
        res.locals.mobileNumber = mobileNumber
        return next()
}
return next({ status: 400, message: `A 'mobileNumber' property is required.`,})
}
...
//middleware for checking if the status is in correct format
function dataStringIsValid(req, res, next) {
    const { data: { status } = {} } = req.body
    //if the dish delivery status is one of the following valid statuses, move
    if(
    ...
    next({ status: 400, message: `status property must be valid string: 'pending', 'preparing', 'out-for-delivery', or 'delivered'`,})
}
...
function dishesArrayIsValid(req, res, next) {
    const { data: { dishes } = {} } = req.body
    // if there are no dishes return the message
    if (!Array.isArray(res.locals.dishes) || res.locals.dishes.length == 0) {
        next({ status: 400, message: `invalid dishes property: dishes property must be non-empty array`,})
    }
    next()
}
...
function dishesArrayLengthIsValid(req, res, next) {
    ...
    dishes.forEach((dish) => {
        ...
        if (!quantity || quantity <= 0 || typeof quantity !== "number") {
            return next({ status: 400, message: `dish ${dish.id} must have quantity property, quantity must be an integer, and it must not be equal to or less than 0`,})
        }
    })
    next()
}
...
function dataIdMatchesOrderId(req, res, next) {
    ...
    if (id!== undefined && id !== null && id !== "" && id !== orderId) {
        next({ status: 400, message: `id ${id} must match orderId provided in parameters`,})
    }
    return next()
}
...
function destroy(req, res, next) {
    ...
    return next({ status: 400, message: `order cannot be deleted unless order status = 'pending'`,})
  }
